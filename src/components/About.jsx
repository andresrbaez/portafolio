import React from 'react';

const About = () => {
    return (
        <div className='aboutMe' id='s-about' style={{paddingTop: "4rem"}}>
            <div>
                <h1 className='h1'>
                    Acerca de mí
                </h1>
            </div>
            <div>
                <h5 className='h5'>
                {/* Desarrollador Full Stack, con experiencia desarrollando y diseñando aplicaciones y plataformas web a través del consumo de API's, garantizando la optimización y el acceso a las mismas a través de diseños responsivos con el usuario. Cuento con conocimientos en HTML, CSS, JavaScript, React JS, Github, Node JS, Express JS, Postman y PostgreSQL. Me caracterizo por ser un profesional con gran capacidad de análisis y adaptabilidad al cambio. Actualmente estoy en busqueda de oportunidades laborales en las que pueda seguir aprendiendo y desarrollando mi pasión por el front-end y sobre nuevas tecnologías para mi crecimiento profesional. */}
                Desarrollador web con inglés B2 y experiencia desarrollando y diseñando aplicaciones y plataformas web a través del consumo de API's, garantizando la optimización y el acceso a las mismas através de diseños responsivos con el usuario. Cuento con conocimientos en HTML, CSS, JavaScript, React JS, Git, Node JS y Express JS. Me caracterizo por ser un profesional con gran capacidad de análisis y adaptabilidad al cambio. Actualmente estoy en busqueda de oportunidades laborales en las que pueda seguir aprendiendo y desarrollando mi pasión por el desarrollo web y sobre nuevas tecnologías para mi crecimiento profesional.
                </h5>
            </div>
            
        </div>
    );
};

export default About;